...
import { ArrowRight, BadgeCheck, Bot, BrainCircuit, CheckCircle2, Clock3, Disc3, FileAudio, FolderKanban, Headphones, Mic2, Music2, PackageCheck, Play, Plus, RadioTower, RotateCcw, SlidersHorizontal, Sparkles, UploadCloud, WandSparkles, Waves } from 'lucide-react';
...
const workflowSteps = [
  ...
  { label: 'Engineer', helper: 'Mix, master, approval', icon: SlidersHorizontal },
...
const creationPaths: Array<{ id: CreationPath; label: string; helper: string; icon: typeof Mic2 }> = [
  ...
  { id: 'voice', label: 'Start from voice', helper: 'Record a demo idea and build around it.', icon: Waves },
  { id: 'beat', label: 'Start from beat', helper: 'Choose genre, BPM, and groove.', icon: Music2 },
  { id: 'stems', label: 'Start from stems', helper: 'Upload tracks for mix and master.', icon: UploadCloud },
...
const getReadiness = (project: Project) => {
  ...
  return { score: Math.round((checks.filter((check) => check.complete).length / checks.length) * 100), missing: checks.filter((check) => !check.complete).slice(0, 2) };
};

const analyseLyrics = (lyrics: string, genre: string, bpm: number) => {
  ...
  ].map((profile) => ({ ...profile, score: profile.terms.reduce((count, term) => count + (lower.includes(term) ? 1 : 0), 0) })).sort((a, b) => b.score - a.score);
  const top = profiles[0].score > 0 ? profiles[0] : { mood: 'focused / cinematic', key: 'F minor', shift: 0 };
  return {
    wordCount: words.length,
    mood: top.mood,
    key: top.key,
    bpm: Math.min(Math.max(bpm + top.shift, 72), 148),
    density: words.length > 130 ? 'dense lyric' : words.length > 60 ? 'balanced lyric' : words.length > 0 ? 'short sketch' : 'waiting for lyrics',
    genre,
    structure: words.length > 120 ? 'Intro → Verse 1 → Pre-Hook → Hook → Verse 2 → Hook → Bridge → Final Hook' : 'Intro → Verse → Hook → Verse → Hook → Outro',
  };
};
...
export default function Dashboard({ projects, onStartProject, onCreateSong }: DashboardProps) {
  ...
  const handleGenreChange = (genreId: string) => { const nextGenre = GENRES.find((genre) => genre.id === genreId) ?? GENRES[0]; setSelectedGenreId(nextGenre.id); setSelectedBeatName(nextGenre.beats[0].name); };
  const handleCreateSong = () => {
    if (creationPath === 'blank') { onStartProject(); return; }
    if (!lyrics.trim()) return;
    onCreateSong({ trackName: trackName.trim() || `Lyrics Session ${projects.length + 1}`, lyrics: lyrics.trim(), selectedGenreId: selectedGenre.id, selectedBeatName, prompt: `${selectedGenre.name} production using ${selectedBeatName}. Copilot: ${copilot.mood}, ${copilot.key}, ${copilot.bpm} BPM, ${copilot.structure}.` });
      ...
      <div className="mx-auto grid min-h-screen max-w-[98rem] gap-6 px-4 py-5 md:px-6 lg:grid-cols-[18rem_1fr] lg:py-6">
        ...
        <section className="space-y-6">
          <section className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
            <div className="relative overflow-hidden rounded-[2.25rem] border border-white/5 bg-glass/85 p-6 shadow-panel backdrop-blur-xl md:p-8"><div className="absolute -right-20 -top-24 h-72 w-72 rounded-full bg-cyan/15 blur-3xl" /><div className="absolute -bottom-28 left-20 h-72 w-72 rounded-full bg-magenta/15 blur-3xl" /><div className="relative"><div className="mb-5 inline-flex items-center gap-2 rounded-full border border-cyan/20 bg-cyan/10 px-3 py-1 text-xs uppercase tracking-[0.28em] text-cyan"><Sparkles className="h-3.5 w-3.5" /> AI song copilot</div><h1 className="max-w-4xl text-4xl font-semibold leading-[0.95] tracking-tight md:text-6xl">Drop an idea. Leave with a finished record plan.</h1><p className="mt-5 max-w-2xl text-base leading-7 text-white/55">AudioMagic now feels less like a static dashboard and more like a studio companion: it reads the song direction, suggests a creative path, tracks readiness, and moves the session from writing to release.</p>
