...
export default function AudioMVP({ open, onClose }) {
  ...
  const readyState = fileId ? 'done' : processing ? 'active' : 'pending';
  ...
  return <div style={panel}><div style={card}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, alignItems: 'flex-start', flexWrap: 'wrap' }}><div><p className="eyebrow">Infinity v10.2</p><h2 style={{ margin: '6px 0 8px', fontSize: 'clamp(26px,4vw,42px)' }}>Before/after Mix & Master workflow</h2><p style={{ color: 'rgba(245,248,255,.68)', lineHeight: 1.7, maxWidth: 900 }}>Upload audio, analyze it, render a v10 FFmpeg master, compare original vs mastered preview, download WAV/MP3/export assets, and save rendered masters into the project library.</p></div><button onClick={onClose} style={{ border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.05)', color: '#f5f8ff', borderRadius: 14, padding: 10 }}><X size={18} /></button></div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 14, marginTop: 16 }}><div className="card" style={{ boxShadow: 'none' }}><h3><FileAudio size={18} /> Source file</h3>{fileInfo ? <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr', gap: 10, color: 'rgba(245,248,255,.68)' }}><span>Name</span><b style={{ color: '#f5f8ff' }}>{fileInfo.name}</b><span>Type</span><b style={{ color: '#f5f8ff' }}>{fileInfo.type}</b><span>Size</span><b style={{ color: '#f5f8ff' }}>{formatBytes(fileInfo.size)}</b><span>File ID</span><b style={{ color: '#f5f8ff', wordBreak: 'break-word' }}>{fileId || 'Not uploaded yet'}</b></div> : <p style={{ color: 'rgba(245,248,255,.68)' }}>No file loaded yet.</p>}<button className="secondary" onClick={exportSession} disabled={!fileInfo} style={{ marginTop: 16 }}><Download size={15} /> Export session JSON</button></div><div className="card" style={{ boxShadow: 'none' }}><h3>Before / After Preview</h3><div className="toggle" style={{ margin: '8px 0 12px' }}><button className={previewMode === 'original' ? 'active' : ''} onClick={() => setPreviewMode('original')}>Original</button><button className={previewMode === 'master' ? 'active' : ''} onClick={() => setPreviewMode('master')} disabled={!masteredPreviewUrl && !masteredMp3Url}>Mastered</button></div>{previewSource ? <audio key={previewSource} controls src={previewSource} style={{ width: '100%' }} /> : <p className="muted">Upload audio to enable preview.</p>}<div className="actions" style={{ marginTop: 12 }}>{audioUrl ? <a className="secondary" href={audioUrl} download={fileInfo?.name || 'original-audio'}><Download size={15}/> Original</a> : null}{masteredWavUrl ? <a className="primary" href={masteredWavUrl} target="_blank" rel="noreferrer"><Download size={15}/> Master WAV</a> : null}{masteredMp3Url ? <a className="secondary" href={masteredMp3Url} target="_blank" rel="noreferrer"><Download size={15}/> Master MP3</a> : null}</div></div></div>
    <div className="card" style={{ boxShadow: 'none', marginTop: 16 }}><h3>Waveform view</h3><Waveform values={waveform} tone={previewMode === 'master' ? '#ff4de1' : '#55e9ff'} /></div>
    ...
    <div className="card" style={{ boxShadow: 'none', marginTop: 16 }}><h3>v10 processing controls</h3><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }}><label className="auth-field">Mode<select value={mode} onChange={(event) => setMode(event.target.value)}><option>Custom AI adaptive</option><option>Trap</option><option>Afrobeat</option><option>Drill</option><option>House</option><option>Gospel</option><option>Cinematic</option><option>Soul</option><option>Experimental</option></select></label><label className="auth-field">Strength {strength}%<input type="range" min="0" max="100" value={strength} onChange={(event) => setStrength(event.target.value)} /></label></div><div className="actions" style={{ marginTop: 14 }}><button className="primary" onClick={() => runBackendJob('mix')} disabled={!fileId || processing}>Build Mix Plan</button><button className="primary" onClick={() => runBackendJob('master')} disabled={!fileId || processing}>Render Master</button><button className="secondary" onClick={() => runBackendJob('stems')} disabled={!fileId || processing}>Separate Stems</button><button className="secondary" onClick={() => runBackendJob('export')} disabled={!fileId || processing}>Create Export Package</button></div></div>
    {exportJob?.result?.downloads ? <div className="card" style={{ boxShadow: 'none', marginTop: 16 }}><h3>v10 export package</h3><div className="actions">{Object.entries(exportJob.result.downloads).map(([key, url]) => <a key={key} className="secondary" href={backendUrl(url)} target="_blank" rel="noreferrer"><Download size={15}/> {key.replaceAll('_', ' ')}</a>)}</div></div> : null}
