...
function Section({ eyebrow, title, text, action, children }) { return <Card><div className="section-head"><div><p className="eyebrow">{eyebrow}</p><h2>{title}</h2><p className="muted wide">{text}</p></div>{action}</div>{children}</Card>; }
...
function makeSoundName(prompt, index = 0) { const words = prompt.trim().split(/\s+/).filter(Boolean).slice(0, 3); const base = words.length ? words.map((word) => word[0].toUpperCase() + word.slice(1)).join(' ') : 'Infinity Sound'; const suffixes = ['Layer', 'Pulse', 'Atmos Pad', 'Texture', 'Hit', 'Loop']; return `${base} ${suffixes[index % suffixes.length]}`; }
function writeString(view, offset, value) { for (let i = 0; i < value.length; i += 1) view.setUint8(offset + i, value.charCodeAt(i)); }

function emitProjectSoundPack(sounds, meta = {}) {
  if (!Array.isArray(sounds) || sounds.length === 0) return;
  const primary = sounds[0];
  const prompt = meta.prompt || primary.prompt || 'Generated sound';
  window.dispatchEvent(new CustomEvent('infinity:project-sound', {
    detail: {
      id: `sound_pack_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      name: `${prompt.slice(0, 42)} sound pack`,
      prompt,
      type: 'Sound pack',
      source: meta.source || (primary.download_url ? 'railway-backend' : 'browser-fallback'),
      genre: meta.genre,
      emotion: meta.emotion,
      intensity: meta.intensity,
      asset_count: sounds.length,
      download_url: primary.download_url,
      preview_url: primary.preview_url,
      assets: sounds.map((sound) => ({
        id: sound.id || sound.asset_id,
...
function downloadBlob(fileName, payload, type = 'application/json') { const blob = payload instanceof Blob ? payload : new Blob([typeof payload === 'string' ? payload : JSON.stringify(payload, null, 2)], { type }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = fileName; anchor.click(); URL.revokeObjectURL(url); }

async function previewGeneratedSound(sound, intensity) {
  if (sound.preview_url) { const audio = new Audio(backendUrl(sound.preview_url)); audio.volume = 0.88; await audio.play(); return true; }
...
function fallbackSounds(prompt, intensity, variation = 0) {
  ...
  return Array.from({ length: 4 }).map((_, index) => ({ id: `${slugify(basePrompt)}-${Date.now()}-${variation}-${index}`, name: variation ? `${makeSoundName(basePrompt, index + variation)} v${variation + 1}` : makeSoundName(basePrompt, index), prompt: variation ? `${basePrompt} variation ${variation + 1}` : basePrompt, type: index % 2 ? 'Loop' : 'Texture', local: true, intensity }));
}

function Overview({ go }) { return <div className="stack"><section className="hero"><motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}><span className="pill">Infinity · evolving AI audio engineer</span><h1>The future of <span>music creation</span> powered by adaptive intelligence.</h1><p className="lead">Infinity is a working beta for private projects, upload analysis, backend mastering, sound generation and structured artist feedback.</p><div className="actions"><button className="primary" onClick={() => go('mix')}><CloudUpload size={18}/> Start Mix & Master</button><button className="secondary" onClick={() => go('generator')}><Sparkles size={18}/> Generate Sounds</button></div><div className="mini-grid"><Card>Upload vocals, instrumentals, full songs or stems</Card><Card>Generate previewable/downloadable WAV textures</Card><Card>Export backend sessions and beta feedback</Card></div></motion.div><Card className="cinema"><div className="orb one"/><div className="orb two"/><div className="orb three"/><h3>Adaptive mastering state</h3><Bars /><div className="signal-grid"><span>104 BPM</span><span>F# minor</span><span>Afro-soul</span><span>-9.4 LUFS</span></div></Card></section><div className="stats"><Card><b>5</b><span>Upload paths</span></Card><Card><b>9</b><span>Mastering modes</span></Card><Card><b>14</b><span>AI analysis stages</span></Card><Card><b>∞</b><span>Learning templates</span></Card></div><Section eyebrow="Core concept" title="One adaptive ecosystem for the full music cycle" text="Infinity replaces fragmented tools with one evolving AI assistant: upload, record, edit, generate, mix, master, export and learn."><div className="three"><Feature icon={SlidersHorizontal} title="AI Mix & Master" text="Backend-connected audio upload, analysis, mix job, FFmpeg mastering and export package."/><Feature icon={Layers3} title="AI DAW" text="Interactive lightweight DAW preview with timeline, live preview tone, session export and workflow staging."/><Feature icon={Sparkles} title="AI Sound Generator" text="Prompt-to-WAV generation with backend assets, preview playback and downloadable WAV files."/></div></Section></div>; }

function MixPage() { const [mode, setMode] = useState('Custom AI adaptive'); const [strength, setStrength] = useState(72); const [compare, setCompare] = useState('After'); return <div className="stack"><Section eyebrow="Section 1" title="AI Mix & Master" text="Drag and drop MP3, WAV, FLAC or STEM ZIP. Infinity detects BPM, key, genre and vocal tone, then aligns, cleans, balances, compresses, widens and masters." action={<button className="primary"><CloudUpload size={16}/> Upload audio</button>}><div className="upload-grid">{uploadTypes.map(([title, Icon, text]) => <div className="upload" key={title}><Icon/><h3>{title}</h3><p>{text}</p></div>)}</div></Section><div className="two"><Section eyebrow="AI pipeline" title="Automatic repair and enhancement" text="This path is connected to the FastAPI audio modal for upload, analysis, mix, master, stems and export."><div className="pipeline">{aiPipeline.map(item => <span key={item}><Zap size={14}/>{item}</span>)}</div></Section><Section eyebrow="Adaptive mastering" title="Genre modes + manual override" text="Select a mastering mode, compare before/after and control AI strength."><div className="chips">{masteringModes.map(item => <button key={item} onClick={() => setMode(item)} className={mode === item ? 'chip active' : 'chip'}>{item}</button>)}</div><div className="toggle"><button onClick={() => setCompare('Before')} className={compare === 'Before' ? 'active' : ''}>Before</button><button onClick={() => setCompare('After')} className={compare === 'After' ? 'active' : ''}>After</button></div><label className="range"><span>AI strength <b>{strength}%</b></span><input type="range" min="0" max="100" value={strength} onChange={(e) => setStrength(e.target.value)} /></label></Section></div><div className="two"><Card><h3>Waveform + EQ movement</h3><Bars tone={compare === 'After' ? 'cyan' : 'magenta'} /></Card><Card><h3>Gain reduction + stereo image</h3><Bars tone="violet" count={28}/><div className="scope">{mode}</div></Card></div></div>; }

function DawPage() { const [status, setStatus] = useState('Ready. Click Live preview to audition the session bed.'); const playPreview = async () => { const ok = await previewGeneratedSound({ name: 'Infinity DAW Session Bed', prompt: 'cinematic studio timeline groove' }, 62); setStatus(ok ? 'Playing DAW session preview.' : 'Browser blocked audio. Click Live preview again or check tab volume.'); }; const exportSession = () => downloadBlob('infinity-daw-session.json', { product: 'Infinity', session: 'AI DAW', tracks: ['Lead Vocal', 'Synth Texture', 'Percussion Loop', 'Atmosphere FX'], exportedAt: new Date().toISOString(), status: 'working-beta-session-export' }); return <div className="stack"><Section eyebrow="Section 2" title="AI DAW / Sound Creator" text="A futuristic lightweight DAW with timeline editing, draggable blocks, cut/crop/fade, piano roll, effects rack, automation lanes, loops, MIDI and live preview." action={<button data-infinity-local-action="true" className="secondary" onClick={playPreview}><Play size={16}/> Live preview</button>}><div className="daw"><div className="timeline"><div className="ruler">{Array.from({ length: 16 }).map((_, i) => <span key={i}>{i + 1}</span>)}</div>{['Lead Vocal', 'Synth Texture', 'Percussion Loop', 'Atmosphere FX'].map((track, i) => <div className="track" key={track}><strong>{track}</strong><div>{Array.from({ length: 4 + i }).map((_, j) => <button data-infinity-local-action="true" className={j % 2 ? 'clip magenta' : 'clip'} key={j} onClick={playPreview}>{j === 1 ? 'AI' : 'Play'}</button>)}</div></div>)}</div><div className="tools"><Feature icon={Music2} title="AI chord suggestions" text="Generate harmonic movement that fits the emotion."/><Feature icon={AudioLines} title="AI melody continuation" text="Continue motifs and toplines intelligently."/><Feature icon={Gauge} title="AI rhythm generation" text="Build groove-aware patterns and beats."/><Feature icon={Settings2} title="Effects rack" text="Design vocal chains, synth patches and cinematic atmospheres."/><button data-infinity-local-action="true" className="secondary" onClick={exportSession}><Download size={15}/> Export DAW session</button><p className="muted">{status}</p></div></div></Section></div>; }
...
function GeneratorPage() {
  ...
  const playUrl = async (url) => { if (!audioRef.current || !url) return false; audioRef.current.src = backendUrl(url); audioRef.current.load(); await audioRef.current.play(); return true; };
  const generate = async () => {
    ...
    try {
      ...
      emitProjectSoundPack(next, { source: 'railway-backend', prompt: basePrompt, genre, emotion, intensity: Number(intensity) });
      const ok = await playUrl(next[0].preview_url);
      setStatus(ok ? `Generated, saved to project, and previewing ${next[0].name}.` : 'Generated backend WAV files and saved them to the project. Click Preview if autoplay was blocked.');
    } catch (error) {
      const next = fallbackSounds(basePrompt, intensity, variation);
      setSounds(next); setActiveSoundId(next[0].id);
      emitProjectSoundPack(next, { source: 'browser-fallback', prompt: basePrompt, genre, emotion, intensity: Number(intensity) });
      const ok = await previewGeneratedSound(next[0], intensity);
      setStatus(ok ? `Backend unavailable. Browser fallback saved to project and playing ${next[0].name}.` : `Backend unavailable: ${error.message}. Browser sound was also blocked, but the fallback pack was saved to project.`);
    } finally { setBusy(false); }
  };
  const regenerate = async () => { setVariation((current) => current + 1); await generate(); };
  const preview = async (sound) => { setActiveSoundId(sound.id); try { const ok = sound.preview_url ? await playUrl(sound.preview_url) : await previewGeneratedSound(sound, intensity); setStatus(ok ? `Previewing ${sound.name}.` : 'Browser audio is blocked. Click again, check volume, or allow audio for this site.'); } catch { setStatus('Preview failed. Try again or download the WAV.'); } };
  const download = (sound) => { setActiveSoundId(sound.id); if (sound.download_url) window.open(backendUrl(sound.download_url), '_blank', 'noopener,noreferrer'); else downloadBlob(`${slugify(sound.name)}.wav`, createSyntheticWavBlob(`${sound.prompt}-${sound.name}`, intensity), 'audio/wav'); setStatus(`Download started for ${sound.name}.`); };
