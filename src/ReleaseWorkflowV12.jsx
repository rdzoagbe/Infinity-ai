...
const floating = {
  ...
  background: 'linear-gradient(135deg,rgba(85,233,255,.18),rgba(255,77,225,.14))',
  color: '#f5f8ff',
  borderRadius: 999,
  padding: '12px 16px',
  fontWeight: 900,
  boxShadow: '0 16px 60px rgba(0,0,0,.36)',
};
...
function slugify(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'infinity-release';
}
...
function Field({ label, children }) {
  return <label className="auth-field">{label}{children}</label>;
}

function StatusPill({ done, children }) {
  const color = done ? '#57f09c' : '#ffcf66';
  return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: `1px solid ${color}66`, background: `${color}18`, color, borderRadius: 999, padding: '7px 10px', fontSize: 12, fontWeight: 900 }}><CheckCircle2 size={14} />{children}</span>;
}
...
export default function ReleaseWorkflowV12() {
  ...
  const [release, setRelease] = useState({
    ...
    isrc: '',
    upc: '',
    distributor: 'DistroKid / TuneCore ready',
    pitch: '',
  });
  ...
  const update = (key, value) => setRelease((current) => ({ ...current, [key]: value }));
  const toggle = (label) => setChecked((current) => ({ ...current, [label]: !current[label] }));
  const exportReleasePackage = () => {
    const payload = {
      product: 'Infinity',
      version: 'v12 Artist Release Workflow',
      release,
    ...
    downloadJson(`${slugify(release.title || 'infinity-release')}-release-package.json`, payload);
  };
  ...
  return <>
    ...
    {open ? <div data-infinity-auth="true" style={overlay}><div style={shell}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, alignItems: 'flex-start', flexWrap: 'wrap' }}><div><p className="eyebrow">Infinity v12</p><h2 style={{ margin: '6px 0 8px', fontSize: 'clamp(26px,4vw,42px)' }}>Artist Release Workflow</h2><p style={{ color: 'rgba(245,248,255,.68)', lineHeight: 1.7, maxWidth: 880 }}>Prepare a release-ready package after mixing and mastering: metadata, artwork placeholder, credits, platform checklist, and exportable release package.</p></div><button type="button" onClick={() => setOpen(false)} style={{ border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.05)', color: '#f5f8ff', borderRadius: 14, padding: 10 }}><X size={18} /></button></div>
      ...
      <section style={{ ...card, marginTop: 16 }}><h3>Platform preparation</h3><div className="actions">{platforms.map((item) => <span key={item} className="pill">{item}</span>)}</div><p className="muted" style={{ marginTop: 12, lineHeight: 1.6 }}>v12 prepares a release package only. It does not submit to Spotify, Apple Music, TikTok, or distributors yet.</p></section>

      <div className="actions" style={{ marginTop: 16 }}><button type="button" className="primary" onClick={exportReleasePackage}><Download size={16} /> Export Release Package JSON</button><button type="button" className="secondary" onClick={() => setOpen(false)}>Close</button></div>
    </div></div> : null}
  </>;
}
