...
type WorkspaceView = 'song' | 'room' | 'copilot' | 'creator' | 'cycle' | 'ops';
...
export default function Workspace({ project, onProjectUpdate }: WorkspaceProps) {
  ...
  const [workspaceView, setWorkspaceView] = useState<WorkspaceView>(project.stems.length > 0 || project.status !== 'Writing' ? 'cycle' : 'song');
    ...
    <div className="rounded-[2rem] border border-white/5 bg-glass/80 p-5 shadow-panel backdrop-blur-xl">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-white/50">Start with Create Song. After generation, you should see sections, stems, take log, producer notes, mix room objects, and release checklist updates.</p>
        </div>
        <button type="button" onClick={() => setWorkspaceView('song')} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-cyan px-5 py-3 font-semibold text-black shadow-cyan"><Music2 className="h-4 w-4" /> Create / rebuild song</button>
      </div>
        ...
        <header className="rounded-[2rem] border border-white/5 bg-glass/80 p-4 shadow-panel backdrop-blur-xl md:p-5">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-magenta/20 bg-magenta/10 px-3 py-1 text-xs uppercase tracking-[0.25em] text-magenta"><Sparkles className="h-3.5 w-3.5" /> AudioMagic · Artist to release cycle</div>
